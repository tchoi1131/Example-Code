/**
 * I would like to show this code because it demonstrates a variety of skills. This program is from one of my projects - Foodi.
 * First, this program calls google maps api to find the Estimated Time of Arrival (ETA) and the corresponding route for the
 * food delivery. Then, it parses the JSON return file to get the data. The ETA is stored in a int variable. The route is
 * stored in an ArrayList<LatLng> data structrue. Every elements in the ArrayList is the intermediate Latitude and Longitude
 * on the route. Then, the route could be visulaized in another Activty by connecting all the points. Finally, if user presses
 * the "Set" button, it will update the real-time database in Firebase.
 * Skills:
 * 1. Basic programming skill in Android.
 * 2. Basic understanding about RESTful APIs and how to call them in a Android application.
 * 3. Parsing JSON files.
 * 4. Basic understanding about Firebase realtime database and how to use it.
 */
